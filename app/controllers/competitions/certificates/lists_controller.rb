...
class Competitions::Certificates::ListsController < CompetitionNestedController
    ...
    if @certificates_list.save
      redirect_to export_competition_certificates_lists_path(
        certificates_list: {
          ...
          series_team_assessment_id: @certificates_list.series_team_assessment_id,
  ...
  def assign_new_resource
    ...
    @certificates_list.series_team_assessment_id    = params[:series_team_assessment_id]
    ...
    rounds.each do |round|
      round.team_assessments_configs.each do |config|
        @series_team_assessments.push([config.round_full_name, config.round_key])
      end
      round.person_assessments_configs.each do |config|
        @series_person_assessments.push([config.round_full_name, config.round_key])
      end
    end
  end
end
